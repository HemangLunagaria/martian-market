import marsJson from "./abi/json/contracts/MartianMarket";
import auctionJson from "./abi/json/contracts/MartianAuction";
import 'regenerator-runtime/runtime';

console.log(auctionJson);
const Web3 = require("web3");

const contractAddress = "0xD9a37A7332Ae6b5208d22074C56926C15C6e8897"; // insert MartianMarket contract address here
...
const dApp = {
  ...
  init: async function() {
    ...
    this.isAdmin = this.accounts[0] == await this.marsContract.methods.getOwner().call();
  ...
  main: async function() {
    // Initialize web3
    //marsJson = await (await fetch(marsJsonFile)).json();
    //auctionJson = await (await fetch(auctionJsonFile)).json();
    await this.init();
    await this.updateUI();
  }
};
